...
testLarge__should_allow_requesting_tasks_from_gradle() {
    ...
    assertEquals 'assemble build buildDependents buildNeeded classes clean jar testClasses init wrapper javadoc buildEnvironment components dependencies dependencyInsight dependentComponents help model projects properties tasks cleanEclipse cleanIdea eclipse idea uploadArchives check test jacocoTestReport sonarqube' "$result"
}
